-- Safe script to add service categories and items
...
-- Check if service_items table exists and what columns it has
-- If it exists but doesn't have service_category_id, add it
DO $$
BEGIN
    -- Check if service_items table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'service_items') THEN
        -- Check if service_category_id column exists
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'service_items' AND column_name = 'service_category_id') THEN
            -- Add the missing column
            ALTER TABLE service_items ADD COLUMN service_category_id UUID REFERENCES service_categories(id);
        END IF;

        -- Check if other required columns exist and add them if missing
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'service_items' AND column_name = 'unit_type') THEN
            ALTER TABLE service_items ADD COLUMN unit_type TEXT NOT NULL DEFAULT 'per_item';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'service_items' AND column_name = 'base_price') THEN
            ALTER TABLE service_items ADD COLUMN base_price DECIMAL(10,2) DEFAULT 0.00;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'service_items' AND column_name = 'is_active') THEN
