-- Safe script to update service categories and items
...
-- Handle existing proposal_line_items by updating them to reference new service items
-- First, let's see what we have and update the references
UPDATE proposal_line_items
SET service_item_id = 'maintenance-custom-schedule'
...
UPDATE proposal_line_items
SET service_item_id = 'office-small'
WHERE service_item_id IN (
  SELECT id FROM service_items WHERE name LIKE '%office%' AND (name LIKE '%small%' OR name LIKE '%2-3%')
);

UPDATE proposal_line_items
SET service_item_id = 'office-medium'
WHERE service_item_id IN (
  SELECT id FROM service_items WHERE name LIKE '%office%' AND (name LIKE '%medium%' OR name LIKE '%4-10%')
);

UPDATE proposal_line_items
SET service_item_id = 'office-large'
WHERE service_item_id IN (
  SELECT id FROM service_items WHERE name LIKE '%office%' AND (name LIKE '%large%' OR name LIKE '%10+%')
);
