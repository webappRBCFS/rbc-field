    ...
    try {
      ...
      if (dsnyResponse.ok) {
        ...
        if (dsnyData && dsnyData.RegularCollectionSchedule) {
          // Parse all collection schedules
          const parseSchedule = (scheduleString) => {
            const days = []
            if (scheduleString.includes('Monday')) days.push('monday')
            if (scheduleString.includes('Tuesday')) days.push('tuesday')
            if (scheduleString.includes('Wednesday')) days.push('wednesday')
            if (scheduleString.includes('Thursday')) days.push('thursday')
            if (scheduleString.includes('Friday')) days.push('friday')
            if (scheduleString.includes('Saturday')) days.push('saturday')
            if (scheduleString.includes('Sunday')) days.push('sunday')
            return days
          }

          const regularSchedule = parseSchedule(dsnyData.RegularCollectionSchedule)
          const bulkSchedule = parseSchedule(dsnyData.BulkPickupCollectionSchedule || '')
          const recyclingSchedule = parseSchedule(dsnyData.RecyclingCollectionSchedule || '')
          const organicsSchedule = parseSchedule(dsnyData.OrganicsCollectionSchedule || '')

          // Primary schedule is regular collection (garbage)
          const pickupDays = regularSchedule
          ...
          const result = {
            address: address,
            pickup_days: pickupDays, // Primary schedule (garbage)
            ...
            schedules: {
              garbage: regularSchedule,
              recycling: recyclingSchedule,
              organics: organicsSchedule,
              bulk: bulkSchedule,
            },
            // Original schedule strings
