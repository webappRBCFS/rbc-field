...
export default function CustomerEdit() {
  ...
  const updateContact = (contactId: string, field: string, value: string) => {
  ...
  const updateProperty = (propId: string, field: string, value: string) => {
    setProperties(properties.map((p) => (p.id === propId ? { ...p, [field]: value } : p)))
  }

  const updatePropertyAddress = (propId: string, addressData: any) => {
    setProperties(
      properties.map((p) =>
        p.id === propId
          ? {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error updating customer: ' + (error as any).message)
    } finally {
      setLoading(false)
    }
  }
                          ...
                          const display =
                            formatted.length > 0
                              ? formatted.length <= 3
                                ...
                                : `(${formatted.slice(0, 3)}) ${formatted.slice(3, 6)}-${formatted.slice(
                                    6
                                  )}`
                              : ''
                            ...
                            setExpandedProperties(
                              new Set(
                                expandedProperties.has(property.id)
                                  ? Array.from(expandedProperties).filter((id) => id !== property.id)
